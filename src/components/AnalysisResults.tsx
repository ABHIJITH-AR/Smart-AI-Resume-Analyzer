...
export const AnalysisResults: React.FC<AnalysisResultsProps> = ({ analysis, onReset }) => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-8 space-y-8">
      {/* Top Bar Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl">
        <div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm text-white bg-[#0877ff] hover:bg-[#0062d6] shadow-md transition-all active:scale-95 cursor-pointer"
          >
          ...
          <button
            onClick={onReset}
            className="px-4 py-2.5 rounded-xl font-semibold text-sm text-slate-300 border border-[#262626] bg-black hover:bg-[#171717] transition-colors cursor-pointer"
          >
            Analyze Another
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-lg flex flex-col items-center justify-center text-center">
        ...
        {/* Quality Score */}
        <div className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-lg flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">
                Quality & Metrics
              </span>
              <Award className="w-5 h-5 text-[#0877ff]" />
            </div>
            <div className="text-3xl font-black text-white">
              {analysis.qualityScore}<span className="text-sm font-normal text-slate-400">/100</span>
            </div>
            <div className="w-full bg-black h-2.5 rounded-full overflow-hidden mt-3 border border-[#262626]">
              <div
                className="bg-[#0877ff] h-full rounded-full transition-all duration-1000"
      ...
      {/* Navigation Sub-Tabs */}
      <div className="flex items-center gap-2 border-b border-[#262626] overflow-x-auto pb-2 scrollbar-none">
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all whitespace-nowrap cursor-pointer ${
            activeTab === 'overview'
              ? 'bg-[#0877ff] text-white shadow-md shadow-[#0877ff]/20'
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-4">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed bg-black p-5 rounded-2xl border border-[#262626]">
          ...
          {analysis.grammarIssues.length > 0 && (
            <div className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-4">
              <h4 className="text-base font-bold text-white flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-[#0877ff]" />
      ...
      {activeTab === 'jobMatch' && analysis.jobMatch && (
        <div className="space-y-6">
          <div className="p-6 sm:p-8 rounded-3xl bg-[#0d0d0d] border border-[#262626] text-white shadow-xl space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="text-center sm:text-right bg-black px-6 py-3 rounded-2xl border border-[#262626]">
                <div className="text-4xl font-black text-[#0877ff]">
                  {analysis.jobMatch.matchPercentage}%
                </div>
                <div className="text-xs text-slate-400 uppercase font-semibold">Match Score</div>
              </div>
            </div>
            <p className="text-sm text-slate-300 leading-relaxed pt-2 border-t border-[#262626]">
            ...
            <div className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {analysis.jobMatch.matchedKeywords.map((kw, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-emerald-950/60 text-emerald-300 border border-emerald-800"
                  >
                    ✓ {kw}
                  </span>
                ))}
            ...
            <div className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {analysis.jobMatch.missingKeywords.map((kw, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-red-950/60 text-red-300 border border-red-800"
                  >
                    + {kw}
                  </span>
                ))}
          ...
          <div className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-4">
            ...
            <ul className="space-y-3">
              {analysis.jobMatch.tailoringAdvice.map((adv, idx) => (
                <li key={idx} className="flex items-start gap-3 text-xs sm:text-sm text-slate-300 p-3 rounded-xl bg-black border border-[#262626]">
                  <span className="w-5 h-5 rounded-full bg-[#0877ff]/20 text-[#0877ff] flex items-center justify-center text-xs font-bold shrink-0">
                    {idx + 1}
                  </span>
                  <span>{adv}</span>
                </li>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {Object.entries(analysis.sections).map(([secKey, rawDetail]) => {
            ...
            return (
              <div
                key={secKey}
                className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-3"
              >
                  ...
                  <div className="pt-2">
                    ...
                    <ul className="space-y-1">
                      {detail.improvements.map((imp, idx) => (
                        <li key={idx} className="text-xs text-[#0877ff] flex items-start gap-1.5">
      ...
      {activeTab === 'skills' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-4">
              <h4 className="text-base font-bold text-white flex items-center gap-2">
                <Code className="w-5 h-5 text-[#0877ff]" />
                Detected Technical Skills
              </h4>
              <div className="flex flex-wrap gap-2">
                {analysis.skills.technicalSkills.map((sk, idx) => (
                  <span key={idx} className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-[#0877ff]/15 text-[#0877ff] border border-[#0877ff]/30">
                    {sk}
                  </span>
                ))}
              </div>
            ...
            <div className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-4">
              <h4 className="text-base font-bold text-white flex items-center gap-2">
                <UserCheck className="w-5 h-5 text-[#0877ff]" />
          ...
          <div className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 rounded-2xl bg-amber-950/30 border border-amber-800/50">
                <p className="text-xs font-bold text-amber-300 uppercase mb-2">
                  Weak / Overused Verbs Found:
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {analysis.actionVerbs.weakVerbsFound.map((v, i) => (
                    <span key={i} className="px-2.5 py-1 rounded-lg text-xs font-mono bg-black text-amber-400 line-through border border-[#262626]">
                      {v}
                    </span>
                  ))}
                </div>
              ...
              <div className="p-4 rounded-2xl bg-emerald-950/30 border border-emerald-800/50">
                <p className="text-xs font-bold text-emerald-300 uppercase mb-2">
                  Suggested High-Impact Action Verbs:
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {analysis.actionVerbs.suggestedStrongVerbs.map((v, i) => (
                    <span key={i} className="px-2.5 py-1 rounded-lg text-xs font-mono font-bold bg-black text-emerald-400 border border-[#262626]">
                      ★ {v}
                    </span>
                  ))}
                </div>
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold bg-black text-[#0877ff] border border-[#0877ff]/30 hover:bg-[#0877ff]/10 transition-colors cursor-pointer"
              >
            ...
            <div className="p-4 rounded-2xl bg-black border border-[#262626] text-slate-200 text-sm font-medium leading-relaxed italic">
              "{analysis.aiRecommendations.summaryRewrite}"
            </div>
          </div>

          {/* Bullet Point Rewrites */}
          <div className="p-6 sm:p-8 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-6">
            <h3 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            <div className="space-y-4">
              {analysis.aiRecommendations.bulletPointRewrites.map((rw, idx) => (
                <div
                  key={idx}
                  className="p-5 rounded-2xl bg-black border border-[#262626] space-y-3"
                >
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1 pt-2 border-t border-[#262626]">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="text-xs text-[#0877ff] hover:underline flex items-center gap-1 font-semibold cursor-pointer"
                      >
                        {copiedBulletIdx === idx ? 'Copied!' : 'Copy'}
                      </button>
                    </div>
                    <p className="text-xs sm:text-sm text-white font-semibold font-mono bg-emerald-950/40 p-2.5 rounded-xl border border-emerald-800">
                      "{rw.improved}"
                    </p>
                  </div>
