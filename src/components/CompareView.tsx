...
export const CompareView: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-8 space-y-8">
      {/* Header */}
      <div className="p-8 rounded-3xl bg-[#0d0d0d] border border-[#262626] text-white shadow-xl space-y-3">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold bg-[#0877ff]/15 text-[#0877ff] border border-[#0877ff]/30">
          <Layers className="w-4 h-4 text-[#0877ff]" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {resumeSlots.map((slot, idx) => (
          <div
            key={slot.id}
            className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-4"
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-[#0877ff] flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-1">
                {resumeSlots.length > 1 ? (
                  <button
                    ...
                    className="px-2 py-1 text-red-400 hover:text-red-300 hover:bg-red-950/50 rounded-lg transition-colors border border-red-900/50 flex items-center gap-1 text-xs font-semibold cursor-pointer"
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 rounded-xl border border-[#262626] bg-black text-sm font-semibold text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#0877ff]/50"
              />
            </div>
            ...
            <div>
              ...
              <div className="mb-2.5">
                <button
                  ...
                  className="w-full py-2.5 px-3 rounded-xl border border-dashed border-[#0877ff]/50 bg-[#0877ff]/10 text-[#0877ff] hover:bg-[#0877ff]/20 text-xs font-bold flex items-center justify-center gap-2 transition-colors cursor-pointer"
                >
                  {slot.loading ? (
                    <div className="w-4 h-4 border-2 border-[#0877ff] border-t-transparent rounded-full animate-spin" />
                  ) : (
                    <>
                      <Upload className="w-4 h-4 text-[#0877ff]" />
              ...
              <textarea
                ...
                className="w-full p-3 rounded-xl border border-[#262626] bg-black text-xs font-mono text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#0877ff]/50"
              />
            </div>
          </div>
        ))}
      ...
      {resumeSlots.length < 4 && (
        <div className="text-center">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold text-[#0877ff] bg-black border border-[#262626] hover:border-[#0877ff] transition-colors shadow-sm cursor-pointer"
          >
      ...
      <div className="p-6 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-xl space-y-3">
        ...
        <textarea
          ...
          className="w-full p-3.5 rounded-xl border border-[#262626] bg-black text-xs sm:text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#0877ff]/50"
        />
      </div>
      ...
      <button
        ...
        className="w-full py-4 rounded-2xl text-base font-bold text-white bg-[#0877ff] hover:bg-[#0062d6] shadow-xl shadow-[#0877ff]/25 active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer"
      >
        {isComparing ? (
          <>
      ...
      {comparison && (
        <div className="p-8 rounded-3xl bg-[#0d0d0d] border border-[#262626] shadow-2xl space-y-6">
          {/* Winner Banner */}
          <div className="p-6 rounded-2xl bg-[#0877ff]/10 border border-[#0877ff]/30 space-y-2">
            <div className="flex items-center gap-2 text-[#0877ff] font-bold text-sm uppercase tracking-wider">
              <Trophy className="w-5 h-5 text-[#0877ff]" />
              ...
              <div
                ...
                className={`p-5 rounded-2xl border ${
                  r.name === comparison.winnerId || r.id === comparison.winnerId
                    ? 'border-[#0877ff] bg-[#0877ff]/15'
                    : 'border-[#262626] bg-black'
                }`}
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  {(r.name === comparison.winnerId || r.id === comparison.winnerId) && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#0877ff] text-white">
                      ★ Winner
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-[#0877ff]">{r.atsScore}/100</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Quality Index:</span>
                    <span className="font-bold text-blue-400">{r.qualityScore}/100</span>
                  </div>
                  {r.matchPercentage !== undefined && (
                    <div className="flex justify-between">
                      <span className="text-slate-400">Job Match:</span>
                      <span className="font-bold text-emerald-400">{r.matchPercentage}%</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <ul className="space-y-2">
              {comparison.keyDifferences.map((diff, idx) => (
                <li key={idx} className="flex items-start gap-2 text-xs sm:text-sm text-slate-300">
                  <CheckCircle2 className="w-4 h-4 text-[#0877ff] shrink-0 mt-0.5" />
                  <span>{diff}</span>
                </li>
              ))}
            </ul>
